...
鉴于上述情况，打开账单文件之后，直接在Excel中删除统计信息行，只保留字段名称和交易内容，然后在“交易创建时间or交易时间”上右键-“排序”-“升序”。
...
Windows下Excel另存后的csv文件会变回GBK编码，而MacOS下修改csv文件并保存不会变更文件的原始编码。为了便于后续操作，需要将非UTF-8文件的编码变更为UTF-8，且换行符变更为LF(\n)，此处以2文件为例，Windows下的3文件同理（处理MacOs下的微信账单3文件时仅需要`cp Csv/2018-01-3.csv Csv/2018-01-3_tmp.csv`代替第一步`iconv`操作）。
